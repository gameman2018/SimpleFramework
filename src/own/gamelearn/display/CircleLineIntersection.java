...
/**
 * Created by kakushouwa on 2017/6/17.
 */
public class CircleLineIntersection extends GameFrameControls {
    private Vector2f p0;
    private Vector2f p1;
    ...
    private void drawIntersections(Graphics g, Vector2f planeIntersection, Vector2f lineIntersection) {
        Matrix3x3f view = getViewportTransform();
        if (planeIntersection != null) {
            g.setColor(Color.BLACK);
            Vector2f intCpy = view.mul(planeIntersection);
            g.drawLine((int) intCpy.x - 20, (int) intCpy.y, (int) intCpy.x + 20, (int) intCpy.y);
            g.drawLine((int) intCpy.x, (int) intCpy.y - 20, (int) intCpy.x, (int) intCpy.y + 20);
        }
        if (lineIntersection != null) {
            g.setColor(Color.BLUE);
            Vector2f intCpy = view.mul(lineIntersection);
            g.drawLine((int)intCpy.x - 20, (int)intCpy.y + 20, (int)intCpy.x + 20, (int)intCpy.y - 20);
        }
    }
}
