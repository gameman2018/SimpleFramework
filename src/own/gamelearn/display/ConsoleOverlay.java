package own.gamelearn.display;

import own.gamelearn.framework.GameFrameControls;
import own.gamelearn.support.Utility;

import java.awt.*;
import java.awt.List;
import java.awt.event.KeyEvent;
import java.awt.image.BufferedImage;
import java.util.*;

/**
 * Created by kakushouwa on 2017/5/20.
 */
public class ConsoleOverlay extends GameFrameControls {
    private BufferedImage console;
    private int xConsole;
    private int yConsole;
    private float alpha;
    private Font consoleFont;
    private int fontHeight;
    private Vector<String> text;
    private float currY;
    private boolean boxes;
    private boolean hide;
    private float hidden;

    public ConsoleOverlay() {
        appWidth = 640;
        appHeight = 640;
        appSleepLong = 10L;
        launchApp(this);
    }

    @Override
    protected void initialize() {
        super.initialize();
        consoleFont = new Font("Courier New", Font.BOLD, 20);
        FontMetrics fm = getFontMetrics(consoleFont);
        fontHeight = fm.getAscent() + fm.getDescent() + fm.getLeading();
        currY = 0;
        text = new Vector<String >();
        text.add("Press the 'H' key to");
        text.add("show and hide the console.");
        text.add("Hover the mouse over the");
        text.add("text to change transparency.");
        text.add("Press Space bar to move text.");
        text.add("Press 'B' to toggle hidden boxes.");
        text.add(0, text.get(text.size() - 1));
        int consoleHeight = fontHeight * (text.size() + 1);
        console = new BufferedImage(appCanvas.getWidth() - 40, consoleHeight, BufferedImage.TYPE_INT_ARGB);
        xConsole = 20;
        yConsole = 20;
        hide = false;
        hidden = 1.0f;
    }

    @Override
    protected void processInput(double delta) {
        super.processInput(delta);
        Point pos = appMouse.getCurPos();
        int minX = xConsole;
        int miny = yConsole;
        int maxX = minX + console.getWidth();
        int maxy = miny + console.getHeight();
        if (pos.x > minX && pos.x < maxX && pos.y > miny && pos.y < maxy) {
            alpha = 1.0f;
        } else {
            alpha = 0.75f;
        }
        if (appKeyboard.keyDownOnce(KeyEvent.VK_SPACE)) {
            text.remove(0);
            text.add(text.get(0));
            currY = fontHeight;
        }
        if (appKeyboard.keyDownOnce(KeyEvent.VK_B)) {
            boxes = !boxes;
        }
        if (appKeyboard.keyDownOnce(KeyEvent.VK_H)) {
            hide = !hide;
        }
    }

    @Override
    protected void updateObjects(double delta) {
        super.updateObjects(delta);
        if (hide && hidden == 1.0f) {
            return;
        }
        if (currY > 0) {
            currY -= fontHeight * delta;
        }
        Graphics2D g = console.createGraphics();
        g.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
        g.setColor(Color.DARK_GRAY);
        g.fillRect(0, 0, console.getWidth(), console.getHeight());
        g.setColor(Color.LIGHT_GRAY);
        g.setFont(consoleFont);
        int x = 20;
        int y = (int) currY;
        Utility.drawString(g,x,y,text);
        if (boxes) {
            g.setColor(Color.DARK_GRAY);
            g.fillRect(0, 0, console.getWidth(), fontHeight);
            g.fillRect(0, fontHeight * text.size(), console.getWidth(), fontHeight);
        } else {
            g.setColor(Color.GREEN);
            g.drawRect(0, 0, console.getWidth() - 1, fontHeight - 1);
            g.drawRect(0, fontHeight * text.size(), console.getWidth() - 1, fontHeight - 1);
        }
        if (hide && hidden < 1.0f) {
            hidden += delta;
            if (hidden > 1.0f) hidden = 1.0f;
        } else if (!hide && hidden > 0.0f) {
            hidden -= delta;
            if (hidden < 0.0f) hidden = 0.0f;
        }
        if (hidden > 0.0f) {
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.CLEAR));
            int xHide = (int) (console.getWidth() * hidden * 0.5f);
            g.fillRect(0, 0, xHide, console.getHeight());//左横刷
            g.fillRect(console.getWidth() - xHide, 0, console.getWidth(), console.getHeight());//右横刷

            int yHide = (int) (console.getHeight() * hidden * 0.5f);
            g.fillRect(0, 0, console.getWidth(), yHide);//上竖刷
            g.fillRect(0, console.getHeight() - yHide, console.getWidth(), console.getHeight());//下竖刷
        }
        g.dispose();
    }

    @Override
    protected void render(Graphics g) {
        super.render(g);
        Graphics2D g2d = (Graphics2D)g;
        int dx = (appCanvas.getWidth()) / 8;
        int dy = (appCanvas.getHeight()) / 8;
        for (int i = 0; i < 8; i++){
            for (int j = 0; j < 8; j++){
                if ((i+j) % 2 == 0){
                    g2d.setColor(Color.BLACK);
                }else {
                    g2d.setColor(Color.WHITE);
                }
                g2d.fillRect(i*dx,j*dy,dx,dy);
            }
        }
        g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,alpha));
        g2d.drawImage(console,xConsole,yConsole,null);
    }
}
