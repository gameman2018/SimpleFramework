...
/**
 * Created by 挨踢狗 on 2017/6/12.
 */
public class LineRectIntersection extends GameFrameControls {
    ...
    protected void processInput(double delta) {
        ...
        end = getViewportReverseTransform().mul(new Vector2f(appMouse.getCurPos().x,(float) appMouse.getCurPos().y));
    ...
    private Float lineRectIntersection(Vector2f O,Vector2f d, Vector2f[] rect){
        ...
        for (int i = 0; i < 2; i++){
            ...
            if (Math.abs(f) > EPSILON){
                ...
                if (largestMin > smallestMax) return null;
                if (smallestMax < 0) return null;
            }else if (e0 * e1 > 0){
                return null;
            }
        }
    ...
    protected void render(Graphics g) {
        ...
        if (hasIntersection){
            ...
            g.drawLine((int)temp.x - 20,(int)temp.y,(int)temp.x+20,(int)temp.y);
            g.drawLine((int)temp.x,(int)temp.y,(int)temp.x-20,(int)temp.y+20);
        }
    }
}
